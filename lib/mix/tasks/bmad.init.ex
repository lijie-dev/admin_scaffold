defmodule Mix.Tasks.Bmad.Init do
  ...
  defp install_git_hooks(opts) do
    ...
    if File.exists?(".git") do
      case :code.priv_dir(:admin_scaffold) do
        {:error, :bad_name} ->
          Mix.shell().info("   ⚠ Hook templates not found (this is expected during development)")
  ...
  defp install_claude_skills(opts) do
    ...
    case :code.priv_dir(:admin_scaffold) do
      {:error, :bad_name} ->
        Mix.shell().info("   ⚠ Skill templates not found (this is expected during development)")
